...
namespace GameServerApp.Handlers
{
    ...
    public class FoodSystemHandler : IMessageHandler
    {
            ...
            HealthActionRequest request)
        {
            ...
            var inventory = await _inventorySystem.GetPlayerInventoryAsync(userName);
            string inventoryItemId = string.IsNullOrWhiteSpace(foodProperties.ItemName)
                ? foodItemId.ToString()
                : foodProperties.ItemName;

            if (inventory.GetItemAmount(inventoryItemId) < 1)
            {
                return (false, "You don't have this food item");
            }

            // Remove food item from inventory
            var removed = inventory.RemoveItem(inventoryItemId, 1);
            if (!removed)
            {
                return (false, "Failed to remove food item from inventory");
            }
            await _inventorySystem.SavePlayerInventoryAsync(userName, inventory);
            ...
            if (!fed)
            {
                ...
                inventory.AddItem(inventoryItemId, 1);
                await _inventorySystem.SavePlayerInventoryAsync(userName, inventory);
