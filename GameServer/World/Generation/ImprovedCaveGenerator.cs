...
namespace GameServerApp.World.Generation
{
    ...
    public sealed class ImprovedCaveGenerator
    {
            ...
            int seaLevel)
        {
            ...
            ApplyFloodplainGroundwaterVaultBridge(mask, hydrologyMask, flowMask, riverMask, heightMap, chunkX, chunkZ, seaLevel);
            ApplyAlluvialAquiferButtressBridge(mask, hydrologyMask, flowMask, riverMask, heightMap, chunkX, chunkZ, seaLevel);
            ApplyKarstFloodplainConduitVaultBridge(mask, hydrologyMask, flowMask, riverMask, heightMap, chunkX, chunkZ, seaLevel);
            ApplyAlluvialSpillConduitSealBridge(mask, hydrologyMask, flowMask, riverMask, heightMap, chunkX, chunkZ, seaLevel);
            ...
            int seaLevel)
        {
            double bridgeWeight = Math.Clamp(
                config.GroundwaterConnectivityWeight * 0.35 +
                config.CaveVentilationBias * 0.33 +
                config.RiparianCaveGuardWeight * 0.32,
                0.0,
                1.25);
            ...
            int bottomClamp = Math.Max(1, seaLevel - Math.Max(5, config.RiparianPlugDepth + 4));
            ...
            for (int x = 1; x < sizeX - 1; x++)
            {
                for (int z = 1; z < sizeZ - 1; z++)
                {
                    ...
                    double relief = Math.Clamp(
                        ...
                        Math.Max(1.0, config.RiparianPlugDepth + 16.0),
                        0.0,
                        1.0);
                    double floodplainBand = Math.Clamp(
                        1.0 - Math.Abs(heightMap[x, z] - seaLevel) / Math.Max(4.0, config.RiparianPlugDepth + 12.0),
                        0.0,
                        1.0);
                    double divergence = Math.Min(1.0, Math.Abs(flow - seamFlow) / divergenceScale);
                    double convergence = Math.Max(0.0, seamFlow - flow) + Math.Max(0.0, seamHydro - hydro);
                    double relayNoise = Math.Abs(SimplexNoise.Generate(
                        (chunkX * sizeX + x) * 0.0022 + 89.0,
                        (chunkZ * sizeZ + z) * 0.0022 - 67.0,
                        1.0,
                        2,
                        1.0,
                        0.55,
                        CreateNoiseSeed(chunkX, chunkZ, x, z, seaLevel, 1741)));
                    ...
                    double sealSignal = Math.Clamp(
                        ...
                        0.0,
                        1.25);
                    sealSignal *= 1.0 + convergence * 0.22 + Math.Clamp((relayNoise - 0.5) * 0.22, -0.16, 0.16);
                    sealSignal *= 1.0 - Math.Clamp(
                        slope * config.CeilingStabilityWeight * 0.018 + relief * 0.34 + divergence * 0.22,
                        0.0,
                        0.86);
                    if (sealSignal <= 0.01)
                    {
                    ...
                    int bottom = Math.Max(bottomClamp, top - Math.Max(5, config.RiparianPlugDepth + 5));
                    if (top <= bottom)
                    {
                        continue;
                    }
                    ...
                    for (int y = top; y >= bottom; y--)
                    {
                        ...
                        double depthFactor = 1.0 - Math.Clamp((top - y) / (double)Math.Max(1, top - bottom), 0.0, 1.0);
                        double sealChance = bridgeWeight * sealSignal * (0.52 + depthFactor * 0.34 + roofThickness * 0.03);
                        if (sealChance > 0.52 || (sealChance > 0.36 && floodplainBand > 0.42))
                        {
                            mask[x, y, z] = false;
                        }
                    }
        ...
        private void ApplyHyporheicExchangeSealBridge(
            bool[,,] mask,
            float[,] hydrologyMask,
            float[,] flowMask,
            float[,]? riverMask,
            ...
            int seaLevel)
        {
            double bridgeWeight = Math.Clamp(
                ...
                config.MoistureRetentionWeight * 0.30,
                0.0,
                1.25);
            if (bridgeWeight <= 0.01)
            {
            ...
            int reliefRadius = Math.Max(2, config.RiparianPlugDepth + 3);
            int topClamp = Math.Min(sizeY - 2, Math.Max(6, seaLevel + Math.Max(4, config.RiparianPlugDepth + 2)));
            int bottomClamp = Math.Max(1, seaLevel - Math.Max(6, config.RiparianPlugDepth + 4));
            double divergenceScale = Math.Max(0.12, config.MoistureFlowClamp * 0.62);
            ...
            for (int x = 1; x < sizeX - 1; x++)
            {
                for (int z = 1; z < sizeZ - 1; z++)
                {
                    ...
                    double relief = Math.Clamp(
                        ...
                        1.0);
                    double floodplainBand = Math.Clamp(
                        1.0 - Math.Abs(heightMap[x, z] - seaLevel) / Math.Max(4.0, config.RiparianPlugDepth + 11.0),
                        0.0,
                        1.0);
                    double divergence = Math.Min(1.0, Math.Abs(flow - seamFlow) / divergenceScale);
                    double exchangeNoise = Math.Abs(SimplexNoise.Generate(
                        (chunkX * sizeX + x) * 0.0022 + 137.0,
                        (chunkZ * sizeZ + z) * 0.0022 - 93.0,
                        1.0,
                        2,
                        1.0,
                        0.55,
                        CreateNoiseSeed(chunkX, chunkZ, x, z, seaLevel, 1879)));
                    ...
                    double exchangeSignal = Math.Clamp(
                        ...
                        0.0,
                        1.25);
                    exchangeSignal *= 1.0 + Math.Clamp((exchangeNoise - 0.5) * 0.22, -0.16, 0.16);
                    ...
                    for (int y = top; y >= bottom; y--)
                    {
                        ...
                        double depthFactor = 1.0 - Math.Clamp((top - y) / (double)Math.Max(1, top - bottom), 0.0, 1.0);
                        double sealChance = bridgeWeight * exchangeSignal * (0.52 + depthFactor * 0.34 + roofThickness * 0.03);
                        if (sealChance > 0.52 || (sealChance > 0.36 && floodplainBand > 0.42))
                        {
                            mask[x, y, z] = false;
                        }
        ...
        private void ApplyKarstFloodplainConduitVaultBridge(
            bool[,,] mask,
            float[,] hydrologyMask,
            float[,] flowMask,
            float[,]? riverMask,
            ...
            int seaLevel)
        {
            ...
            int sizeZ = mask.GetLength(2);
            int reliefRadius = Math.Max(2, config.RiparianPlugDepth + 3);
            int floorY = Math.Max(2, config.RegionalMainCaveMinY);
            double divergenceScale = Math.Max(0.12, config.MoistureFlowClamp * 0.62);
            ...
            for (int x = 1; x < sizeX - 1; x++)
            {
                for (int z = 1; z < sizeZ - 1; z++)
                {
                    ...
                    double slope = TerrainMaskUtility.ComputeSlope(heightMap, x, z);
                    double relief = Math.Clamp(
                        TerrainMaskUtility.ComputeLocalRelief(heightMap, x, z, reliefRadius) / Math.Max(1.0, config.RiparianPlugDepth + 16.0),
                        0.0,
                        1.0);
                    double floodplainBand = Math.Clamp(
                    ...
                    double divergence = Math.Min(1.0, Math.Abs(flow - seamFlow) / divergenceScale);
                    double relayNoise = Math.Abs(SimplexNoise.Generate(
                        (chunkX * sizeX + x) * 0.0021 + 73.0,
                        (chunkZ * sizeZ + z) * 0.0021 - 47.0,
                        1.0,
                        2,
                        1.0,
                        0.55,
                        CreateNoiseSeed(chunkX, chunkZ, x, z, seaLevel, 1523)));

                    double vaultSignal = Math.Clamp(
                        hydro * 0.28 +
                        seamHydro * 0.2 +
                        flow * 0.14 +
                        seamFlow * 0.12 +
                        ...
                        0.0,
                        1.25);
                    vaultSignal *= 1.0 + Math.Clamp((relayNoise - 0.5) * config.CaveVentilationBias * 0.22, -0.16, 0.16);
                    vaultSignal *= 1.0 - Math.Clamp(
                        slope * 0.02 + relief * 0.32 + divergence * 0.24,
                        0.0,
                        0.85);
                    if (vaultSignal <= 0.01)
                    {
                    ...
                    int surface = Math.Max(floorY + 12, Math.Min(sizeY - 2, heightMap[x, z]));
                    int sealTop = Math.Clamp(seaLevel - 3, floorY + 6, Math.Max(floorY + 6, surface - 4));
                    int sealBottom = Math.Max(floorY + 3, sealTop - 5);
                    int conduitBottom = Math.Max(floorY + 1, sealBottom - 4);
                    int conduitTop = Math.Max(conduitBottom, sealBottom - 1);

                    for (int y = sealBottom; y <= sealTop; y++)
                    {
                        if (y < 0 || y >= sizeY || !mask[x, y, z])
                        {
                            continue;
                        }

                        double sealPulse = Math.Abs(SimplexNoise.Generate(
                            (chunkX * sizeX + x) * 0.0031 + y * 0.0061,
                            (chunkZ * sizeZ + z) * 0.0031 - y * 0.0047,
                            1.0,
                            2,
                            1.0,
                            0.55,
                            CreateNoiseSeed(chunkX, chunkZ, x, z, y, 1531)));
                        double sealThreshold = 0.82 - vaultSignal * bridgeWeight * 0.2;
                        if (sealPulse > sealThreshold)
                        {
                            mask[x, y, z] = false;
                        }
                    }
                    ...
                    for (int y = conduitBottom; y <= conduitTop; y++)
                    {
                        ...
                        double conduitPulse = Math.Abs(SimplexNoise.Generate(
                            (chunkX * sizeX + x) * 0.0027 + y * 0.0051,
                            (chunkZ * sizeZ + z) * 0.0027 - y * 0.0039,
                            1.0,
                            2,
                            1.0,
                            0.55,
                            CreateNoiseSeed(chunkX, chunkZ, x, z, y, 1543)));
                        double conduitThreshold = 0.86 - vaultSignal * bridgeWeight * 0.22;
                        if (conduitPulse > conduitThreshold)
                        {
                            mask[x, y, z] = true;
                        }
                    }
        ...
        private void ApplyFloodplainGroundwaterVaultBridge(
            bool[,,] mask,
            float[,] hydrologyMask,
            float[,] flowMask,
            float[,]? riverMask,
            ...
            int seaLevel)
        {
            ...
            int reliefRadius = Math.Max(2, config.RiparianPlugDepth + 3);
            int topClamp = Math.Min(sizeY - 2, Math.Max(6, seaLevel + Math.Max(4, config.RiparianPlugDepth + 2)));
            int bottomClamp = Math.Max(1, seaLevel - Math.Max(5, config.RiparianPlugDepth + 3));
            double divergenceScale = Math.Max(0.12, config.MoistureFlowClamp * 0.6);
            ...
            for (int x = 1; x < sizeX - 1; x++)
            {
                for (int z = 1; z < sizeZ - 1; z++)
                {
                    ...
                    double relief = Math.Clamp(
                        TerrainMaskUtility.ComputeLocalRelief(heightMap, x, z, reliefRadius) /
                        Math.Max(1.0, config.RiparianPlugDepth + 14.0),
                        0.0,
                        1.0);
                    double floodplainBand = Math.Clamp(
                        1.0 - Math.Abs(heightMap[x, z] - seaLevel) / Math.Max(4.0, config.RiparianPlugDepth + 10.0),
                        0.0,
                        1.0);
                    double divergence = Math.Min(1.0, Math.Abs(flow - seamFlow) / divergenceScale);
                    double relayNoise = Math.Abs(SimplexNoise.Generate(
                        (chunkX * sizeX + x) * 0.0021 + 19.0,
                        (chunkZ * sizeZ + z) * 0.0021 - 27.0,
                        1.0,
                        2,
                        1.0,
                        0.55,
                        CreateNoiseSeed(chunkX, chunkZ, x, z, seaLevel, 1259)));
                    ...
                    double vaultSignal = Math.Clamp(
                        ...
                        0.0,
                        1.25);
                    vaultSignal *= 1.0 + Math.Clamp((relayNoise - 0.5) * 0.22, -0.16, 0.16);
                    vaultSignal *= 1.0 - Math.Clamp(
                        slope * config.CeilingStabilityWeight * 0.018 + relief * 0.34 + divergence * 0.2,
                        0.0,
                        0.85);
                    if (vaultSignal <= 0.01)
                    {
                    ...
                    int surface = Math.Clamp(heightMap[x, z], 3, sizeY - 2);
                    int top = Math.Min(topClamp, surface - 1);
                    int bottom = Math.Max(bottomClamp, top - Math.Max(4, config.RiparianPlugDepth + 4));
                    if (top <= bottom)
                    {
                        continue;
                    }
                    ...
                    for (int y = top; y >= bottom; y--)
                    {
                        ...
                        double depthFactor = 1.0 - Math.Clamp((top - y) / (double)Math.Max(1, top - bottom), 0.0, 1.0);
                        double sealChance = bridgeWeight * vaultSignal * (0.54 + depthFactor * 0.32 + roofThickness * 0.03);
                        if (sealChance > 0.52 || (sealChance > 0.36 && floodplainBand > 0.42))
                        {
                            mask[x, y, z] = false;
                        }
        ...
        private void ApplyAlluvialAquiferButtressBridge(
            bool[,,] mask,
            float[,] hydrologyMask,
            float[,] flowMask,
            float[,]? riverMask,
            ...
            int seaLevel)
        {
            double bridgeWeight = Math.Clamp(
                config.AquiferBarrierWeight * 0.38 +
                config.GroundwaterConnectivityWeight * 0.34 +
                config.CeilingStabilityWeight * 0.28,
                0.0,
                1.25);
            if (bridgeWeight <= 0.01)
            {
            ...
            for (int x = 1; x < sizeX - 1; x++)
            {
                for (int z = 1; z < sizeZ - 1; z++)
                {
                    ...
                    double relayNoise = Math.Abs(SimplexNoise.Generate(
                        ...
                        (chunkZ * sizeZ + z) * 0.0022 - 33.0,
                        1.0,
                        2,
                        1.0,
                        0.55,
                        CreateNoiseSeed(chunkX, chunkZ, x, z, seaLevel, 1379)));

                    double buttressSignal = Math.Clamp(
                        hydro * 0.24 +
                        seamHydro * 0.2 +
                        flow * 0.18 +
                        seamFlow * 0.16 +
                        ...
                        0.0,
                        1.25);
                    buttressSignal *= 1.0 + Math.Clamp((relayNoise - 0.5) * 0.22, -0.16, 0.16);
                    buttressSignal *= 1.0 - Math.Clamp(
                        slope * config.CeilingStabilityWeight * 0.018 + relief * 0.34 + divergence * 0.2,
                        0.0,
                        0.85);
                    if (buttressSignal <= 0.01)
                    {
                        continue;
                    }
                    ...
                    for (int y = top; y >= bottom; y--)
                    {
                        ...
                        double depthFactor = 1.0 - Math.Clamp((top - y) / (double)Math.Max(1, top - bottom), 0.0, 1.0);
                        double sealChance = bridgeWeight * buttressSignal * (0.52 + depthFactor * 0.34 + roofThickness * 0.03);
                        if (sealChance > 0.52 || (sealChance > 0.36 && floodplainBand > 0.42))
                        {
                            mask[x, y, z] = false;
                        }
        ...
        private void ApplyAlluvialSpillConduitSealBridge(
            bool[,,] mask,
            float[,] hydrologyMask,
            float[,] flowMask,
            float[,]? riverMask,
            ...
            int seaLevel)
        {
            ...
            for (int x = 1; x < sizeX - 1; x++)
            {
                for (int z = 1; z < sizeZ - 1; z++)
                {
                    ...
                    double relief = Math.Clamp(
                        TerrainMaskUtility.ComputeLocalRelief(heightMap, x, z, reliefRadius) /
                        Math.Max(1.0, config.RiparianPlugDepth + 15.0),
                        0.0,
                        1.0);
                    double floodplainBand = Math.Clamp(
                        1.0 - Math.Abs(heightMap[x, z] - seaLevel) / Math.Max(4.0, config.RiparianPlugDepth + 12.0),
                    ...
                    double divergence = Math.Min(1.0, Math.Abs(flow - seamFlow) / divergenceScale);
                    double relayNoise = Math.Abs(SimplexNoise.Generate(
                        (chunkX * sizeX + x) * 0.0023 + 61.0,
                        (chunkZ * sizeZ + z) * 0.0023 - 43.0,
                        1.0,
                        2,
                        1.0,
                        0.55,
                        CreateNoiseSeed(chunkX, chunkZ, x, z, seaLevel, 1493)));
                    ...
                    double sealSignal = Math.Clamp(
                        ...
                        0.0,
                        1.25);
                    sealSignal *= 1.0 + Math.Clamp((relayNoise - 0.5) * 0.22, -0.16, 0.16);
                    sealSignal *= 1.0 - Math.Clamp(
                        slope * config.CeilingStabilityWeight * 0.018 + relief * 0.34 + divergence * 0.2,
                        0.0,
                        0.86);
                    ...
                    for (int y = top; y >= bottom; y--)
                    {
                        ...
                        double depthFactor = 1.0 - Math.Clamp((top - y) / (double)Math.Max(1, top - bottom), 0.0, 1.0);
                        double sealChance = bridgeWeight * sealSignal * (0.52 + depthFactor * 0.33 + roofThickness * 0.03);
                        if (sealChance > 0.52 || (sealChance > 0.36 && floodplainBand > 0.42))
                        {
                            mask[x, y, z] = false;
                        }
        ...
        private void ApplyFloodplainVentPressureBridge(
            bool[,,] mask,
            float[,] hydrologyMask,
            float[,] flowMask,
            float[,]? riverMask,
            ...
            for (int x = 1; x < sizeX - 1; x++)
            {
                for (int z = 1; z < sizeZ - 1; z++)
                {
                    ...
                    double floodplainBand = Math.Clamp(
                        ...
                        1.0);
                    double divergence = Math.Min(1.0, Math.Abs(flow - seamFlow) / divergenceScale);
                    double pulseNoise = Math.Abs(SimplexNoise.Generate(
                        (chunkX * sizeX + x) * 0.0022 + 73.0,
                        (chunkZ * sizeZ + z) * 0.0022 - 49.0,
                        1.0,
                        2,
                        1.0,
                        0.55,
                        CreateNoiseSeed(chunkX, chunkZ, x, z, seaLevel, 1621)));

                    double ventSignal = Math.Clamp(
                        hydro * 0.22 +
                        seamHydro * 0.2 +
                        flow * 0.18 +
                        seamFlow * 0.16 +
                        ...
                        0.0,
                        1.25);
                    ventSignal *= 1.0 + Math.Clamp((pulseNoise - 0.5) * config.CaveVentilationBias * 0.24, -0.16, 0.16);
                    ventSignal *= 1.0 - Math.Clamp(
                        slope * config.CeilingStabilityWeight * 0.018 + relief * 0.34 + divergence * 0.21,
                        0.0,
                        0.86);
                    if (ventSignal <= 0.01)
                    {
                    ...
                    for (int y = top; y >= bottom; y--)
                    {
                        ...
                        double depthFactor = 1.0 - Math.Clamp((top - y) / (double)Math.Max(1, top - bottom), 0.0, 1.0);
                        double sealChance = bridgeWeight * ventSignal * (0.52 + depthFactor * 0.34 + roofThickness * 0.03);
                        if (sealChance > 0.52 || (sealChance > 0.36 && floodplainBand > 0.42))
                        {
                            mask[x, y, z] = false;
                        }
