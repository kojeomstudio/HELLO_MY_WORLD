...
namespace GameServerApp.World.Generation
{
    ...
    public sealed class ImprovedRiverGenerator
    {
            ...
            int seaLevel)
        {
            ...
            ApplyKarstSpringConfluenceRelayBridge(mask, hydrologyMask, flowAccumulation, heightMap, chunkX, chunkZ, seaLevel);
            ApplyKarstFloodplainConduitBridge(mask, hydrologyMask, flowAccumulation, heightMap, chunkX, chunkZ, seaLevel);
            ApplyAlluvialAnabranchExchangeBridge(mask, hydrologyMask, flowAccumulation, heightMap, chunkX, chunkZ, seaLevel);
        ...
        private void ApplyHyporheicExchangeRelayBridge(
            float[,] mask,
            float[,] hydrology,
            float[,] flow,
            int[,] heightMap,
            ...
            int seaLevel)
        {
            double relayWeight = Math.Clamp(
                ...
                config.HydrologyFlowPersistence * 0.31,
                0.0,
                1.25);
            if (relayWeight <= 0.01)
            {
            ...
            double divergenceScale = Math.Max(0.12, config.HydrologyFlowDivergenceClamp * 0.58);
            ...
            for (int x = 1; x < sizeX - 1; x++)
            {
                for (int z = 1; z < sizeZ - 1; z++)
                {
                    ...
                    double floodplainBand = Math.Clamp(
                        1.0 - Math.Abs(heightMap[x, z] - seaLevel) / Math.Max(4.0, config.RiverMouthSmoothRadius * 1.9),
                        0.0,
                        1.0);
                    ...
                    double relayNoise = Math.Abs(SimplexNoise.Generate(
                        (chunkX * sizeX + x) * 0.0021 + 149.0,
                        (chunkZ * sizeZ + z) * 0.0021 - 87.0,
                        1.0,
                        2,
                        1.0,
                        0.55,
                        CreateNoiseSeed(chunkX, chunkZ, x, z, 1931)));
                    ...
                    double relaySignal = Math.Clamp(
                        ...
                        0.0,
                        1.35);
                    relaySignal *= 1.0 + Math.Clamp((relayNoise - 0.5) * config.RiverMeanderJitter * 0.2, -0.15, 0.15);
                    relaySignal *= 1.0 - Math.Clamp(
                        slope * config.HydrologySlopePenalty * 0.013 + relief * 0.31 + divergence * 0.24,
                        0.0,
                        0.86);
                    if (relaySignal <= 0.01)
                    {
                    ...
                    double continuityFloor = Math.Max(river * (0.87 + config.RiverEdgeContinuityWeight * 0.08), relaySignal * 0.18);
                    double target = river * (1.0 - relayWeight * 0.12) + (river + relaySignal) * relayWeight * 0.12;
                    mask[x, z] = (float)Math.Clamp(Math.Max(target, continuityFloor), 0.0, 1.35);
                }
            }
        }
        ...
        private void ApplyKarstSpringConfluenceRelayBridge(
            float[,] mask,
            float[,] hydrology,
            float[,] flow,
            int[,] heightMap,
        ...
        {
            double relayWeight = Math.Clamp(
                config.RiverTributaryCaptureWeight * 0.36 +
                config.HydrologyFlowPersistence * 0.34 +
                config.RiverConfluenceBoost * 0.30,
                0.0,
                1.25);
            if (relayWeight <= 0.01)
            {
            ...
            for (int x = 1; x < sizeX - 1; x++)
            {
                for (int z = 1; z < sizeZ - 1; z++)
                {
                    ...
                    double convergence = Math.Max(0.0, seamFlow - flowNode) + Math.Max(0.0, seamHydro - hydro);
                    double relayNoise = Math.Abs(SimplexNoise.Generate(
                        (chunkX * sizeX + x) * 0.0022 + 37.0,
                        (chunkZ * sizeZ + z) * 0.0022 - 29.0,
                        1.0,
                        2,
                        1.0,
                        0.55,
                        CreateNoiseSeed(chunkX, chunkZ, x, z, 1297)));
                    double relaySignal = Math.Clamp((hydro + seamHydro + flowNode + seamFlow) * 0.25, 0.0, 1.2);
                    relaySignal *= 1.0 + Math.Clamp((relayNoise - 0.5) * 0.22, -0.16, 0.16);
                    relaySignal *= 1.0 + convergence * 0.22 + floodplainBand * 0.16;
                    relaySignal *= 1.0 - Math.Clamp(
                        slope * config.HydrologySlopePenalty * 0.014 + relief * 0.32 + divergence * 0.22,
                        0.0,
                        0.85);
                    ...
                    double floor = Math.Max(river * (0.86 + config.RiverEdgeContinuityWeight * 0.08), relaySignal * 0.17);
                    double target = river * (1.0 - relayWeight * 0.12) + (river + relaySignal) * relayWeight * 0.12;
                    mask[x, z] = (float)Math.Clamp(Math.Max(target, floor), 0.0, 1.35);
                }
            }
        ...
        private void ApplyKarstFloodplainConduitBridge(
            float[,] mask,
            float[,] hydrology,
            float[,] flow,
            int[,] heightMap,
            ...
            int seaLevel)
        {
            double conduitWeight = Math.Clamp(
                config.RiverConfluenceBoost * 0.34 +
                config.RiverDeltaWetlandStrength * 0.33 +
                config.HydrologyFlowPersistence * 0.33,
                0.0,
                1.25);
            if (conduitWeight <= 0.01)
            {
                return;
            }
            ...
            for (int x = 1; x < sizeX - 1; x++)
            {
                for (int z = 1; z < sizeZ - 1; z++)
                {
                    ...
                    double divergence = Math.Min(1.0, Math.Abs(flowNode - seamFlow) / divergenceScale);
                    double relayNoise = Math.Abs(SimplexNoise.Generate(
                        (chunkX * sizeX + x) * 0.0021 + 59.0,
                        (chunkZ * sizeZ + z) * 0.0021 - 41.0,
                        1.0,
                        2,
                        1.0,
                        0.55,
                        CreateNoiseSeed(chunkX, chunkZ, x, z, 1409)));

                    double conduitSignal = Math.Clamp(
                        river * 0.36 +
                        hydro * 0.2 +
                        seamHydro * 0.16 +
                        flowNode * 0.14 +
                        ...
                        0.0,
                        1.35);
                    conduitSignal *= 1.0 + Math.Clamp((relayNoise - 0.5) * config.RiverMeanderJitter * 0.2, -0.16, 0.16);
                    conduitSignal *= 1.0 - Math.Clamp(
                        slope * config.HydrologySlopePenalty * 0.013 + relief * 0.31 + divergence * 0.25,
                        0.0,
                        0.86);
                    if (conduitSignal <= 0.01)
                    {
                        continue;
                    }

                    double continuityFloor = Math.Max(river * (0.87 + config.RiverEdgeContinuityWeight * 0.08), conduitSignal * 0.18);
                    double target = river * (1.0 - conduitWeight * 0.12) + (river + conduitSignal) * conduitWeight * 0.12;
                    mask[x, z] = (float)Math.Clamp(Math.Max(target, continuityFloor), 0.0, 1.35);
                }
            }
        }

        private void ApplyAlluvialAnabranchExchangeBridge(
            float[,] mask,
            float[,] hydrology,
            float[,] flow,
            int[,] heightMap,
            ...
            int seaLevel)
        {
            ...
            for (int x = 1; x < sizeX - 1; x++)
            {
                for (int z = 1; z < sizeZ - 1; z++)
                {
                    ...
                    double divergence = Math.Min(1.0, Math.Abs(flowNode - seamFlow) / divergenceScale);
                    double relayNoise = Math.Abs(SimplexNoise.Generate(
                        (chunkX * sizeX + x) * 0.0023 + 67.0,
                        (chunkZ * sizeZ + z) * 0.0023 - 35.0,
                        1.0,
                        2,
                        1.0,
                        0.55,
                        CreateNoiseSeed(chunkX, chunkZ, x, z, 1537)));
                    double exchangeSignal = Math.Clamp(
                        river * 0.35 +
                        hydro * 0.2 +
                        seamHydro * 0.16 +
                        ...
                        0.0,
                        1.35);
                    exchangeSignal *= 1.0 + Math.Clamp((relayNoise - 0.5) * config.RiverMeanderJitter * 0.22, -0.16, 0.16);
                    ...
                    double continuityFloor = Math.Max(river * (0.87 + config.RiverEdgeContinuityWeight * 0.08), exchangeSignal * 0.18);
                    double target = river * (1.0 - exchangeWeight * 0.12) + (river + exchangeSignal) * exchangeWeight * 0.12;
                    mask[x, z] = (float)Math.Clamp(Math.Max(target, continuityFloor), 0.0, 1.35);
                }
            }
        ...
        private void ApplyFloodplainSpringPulseAnchorBridge(
            float[,] mask,
            float[,] hydrology,
            float[,] flow,
            int[,] heightMap,
            ...
            for (int x = 1; x < sizeX - 1; x++)
            {
                for (int z = 1; z < sizeZ - 1; z++)
                {
                    ...
                    double pulseNoise = Math.Abs(SimplexNoise.Generate(
                        (chunkX * sizeX + x) * 0.0022 + 79.0,
                        (chunkZ * sizeZ + z) * 0.0022 - 57.0,
                        1.0,
                        2,
                        1.0,
                        0.55,
                        CreateNoiseSeed(chunkX, chunkZ, x, z, 1667)));

                    double pulseSignal = Math.Clamp(
                        river * 0.34 +
                        hydro * 0.18 +
                        seamHydro * 0.15 +
                        flowNode * 0.15 +
                        ...
                        0.0,
                        1.35);
                    pulseSignal *= 1.0 + Math.Clamp((pulseNoise - 0.5) * (config.RiverMeanderJitter + 0.12) * 0.18, -0.15, 0.15);
                    pulseSignal *= 1.0 - Math.Clamp(
                        slope * config.HydrologySlopePenalty * 0.013 + relief * 0.3 + divergence * 0.24,
                        0.0,
                        0.86);
                    if (pulseSignal <= 0.01)
                    {
                    ...
                    double continuityFloor = Math.Max(river * (0.87 + config.RiverEdgeContinuityWeight * 0.08), pulseSignal * 0.18);
                    double target = river * (1.0 - bridgeWeight * 0.12) + (river + pulseSignal) * bridgeWeight * 0.12;
                    mask[x, z] = (float)Math.Clamp(Math.Max(target, continuityFloor), 0.0, 1.35);
                }
            }
        }

        private void ApplySubsurfaceConfluenceStabilityBridge(
            float[,] mask,
            float[,] hydrology,
            float[,] flow,
            int[,] heightMap,
            ...
            int seaLevel)
        {
            ...
            int sizeX = mask.GetLength(0);
            int sizeZ = mask.GetLength(1);
            int reliefRadius = Math.Max(2, config.HydrologyWatershedStitchRadius + 3);
            double divergenceScale = Math.Max(0.12, config.HydrologyFlowDivergenceClamp * 0.6);
            var copy = (float[,])mask.Clone();
            ...
            for (int x = 1; x < sizeX - 1; x++)
            {
                for (int z = 1; z < sizeZ - 1; z++)
                {
                    ...
                    double relief = Math.Clamp(
                        TerrainMaskUtility.ComputeLocalRelief(heightMap, x, z, reliefRadius) /
                        Math.Max(1.0, config.HydrologyWaterTableClampRange + 9.0),
                        0.0,
                        1.0);
                    double floodplainBand = Math.Clamp(
                        1.0 - Math.Abs(heightMap[x, z] - seaLevel) / Math.Max(4.0, config.RiverMouthSmoothRadius * 1.9),
                    ...
                    double conduitNoise = Math.Abs(SimplexNoise.Generate(
                        (chunkX * sizeX + x) * 0.0021 + 113.0,
                        (chunkZ * sizeZ + z) * 0.0021 - 71.0,
                        1.0,
                        2,
                        1.0,
                        0.55,
                        CreateNoiseSeed(chunkX, chunkZ, x, z, 1729)));

                    double stabilitySignal = Math.Clamp(
                        river * 0.34 +
                        hydro * 0.2 +
                        seamHydro * 0.16 +
                        flowNode * 0.16 +
                        ...
                        0.0,
                        1.35);
                    stabilitySignal *= 1.0 + convergence * 0.2 + Math.Clamp((conduitNoise - 0.5) * 0.2, -0.15, 0.15);
                    stabilitySignal *= 1.0 - Math.Clamp(
                        slope * config.HydrologySlopePenalty * 0.013 + relief * 0.31 + divergence * 0.24,
                        0.0,
                        0.86);
                    if (stabilitySignal <= 0.01)
                    {
                        continue;
                    }

                    double continuityFloor = Math.Max(river * (0.87 + config.RiverEdgeContinuityWeight * 0.08), stabilitySignal * 0.18);
                    double target = river * (1.0 - bridgeWeight * 0.12) + (river + stabilitySignal) * bridgeWeight * 0.12;
                    mask[x, z] = (float)Math.Clamp(Math.Max(target, continuityFloor), 0.0, 1.35);
                }
            }
        }
