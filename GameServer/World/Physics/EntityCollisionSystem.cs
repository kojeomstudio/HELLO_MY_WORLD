...
namespace GameServerApp.World.Physics
{
    ...
    public class EntityCollisionSystem
    {
        ...
        public EntityCollisionSystem(ILogger<EntityCollisionSystem> logger, WorldGenerationConfig config)
        {
            ...
            this.collisionConfig = config.Physics.Collision ?? new EntityCollisionConfig();
            ...
            var worldSize = config.World.WorldSize * config.World.ChunkSize;
        ...
        {
            var chunkX = (int)Math.Floor(position.X / config.World.ChunkSize);
            var chunkZ = (int)Math.Floor(position.Z / config.World.ChunkSize);
            var chunkPos = new Vector3Int(chunkX, 0, chunkZ);
            ...
            var localX = (int)Math.Floor(position.X) - chunkX * config.World.ChunkSize;
            var localZ = (int)Math.Floor(position.Z) - chunkZ * config.World.ChunkSize;
            
            // Check bounds
            if (localX < 0 || localX >= config.World.ChunkSize || 
                localZ < 0 || localZ >= config.World.ChunkSize)
            {
                return null;
            }
        ...
        {
            var size = config.World.ChunkSize;
