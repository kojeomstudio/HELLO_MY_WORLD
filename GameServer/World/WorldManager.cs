...
namespace GameServerApp.World
{
    public partial class WorldManager
    {
        ...
        private readonly double _hydrologyFlowMemoryWeight;
        ...
        private readonly double _riparianCaveGuardWeight;
        ...
        public WorldManager(DatabaseHelper database, WorldSettings? worldSettings = null, WorldGenerationConfig? generationConfig = null, int worldId = 1, WorldSeedConfig? worldSeed = null)
        {
            ...
            _hydrologyFlowMemoryWeight = Math.Clamp(_worldGenConfig.Water.HydrologyFlowMemoryWeight, 0.0, 1.0);
            ...
            _riparianCaveGuardWeight = Math.Clamp(_worldGenConfig.Caves.RiparianCaveGuardWeight, 0.0, 1.0);
            ...
            Vector2[,] hydrologyGradient)
        {
            ...
            for (int x = 0; x < 16; x++)
            {
                for (int z = 0; z < 16; z++)
                {
                    ...
                    double riparianGuard = Math.Clamp(Math.Max(hydrology, flow) * _riparianCaveGuardWeight, 0.0, 1.0);
                    double riverGuard = Math.Clamp((1.0 - riverPressure) * _riparianCaveGuardWeight * 0.5, 0.0, 0.6);
                    double edgeGuard = Math.Clamp(gradientStrength * _riparianCaveGuardWeight * 0.25, 0.0, 0.35);
                    double guardMultiplier = 1.0 - Math.Clamp(riparianGuard * 0.35 + riverGuard * 0.25 + edgeGuard * 0.2, 0.0, 0.75);
                    ...
                    stability[x, z] = Math.Clamp(stabilityValue * guardMultiplier, 0.0, 1.0);
                }
            }
        ...
        private void StabilizeMoistCaveCeilings(
            ...
            double[,] flowAccumulation)
        {
            double stabilityWeight = Math.Clamp(_caveCeilingStabilityWeight, 0.0, 1.0);
            double moistureClamp = Math.Max(0.05, _caveCeilingMoistureClamp);
            ...
            for (int x = 1; x < 15; x++)
            {
                for (int z = 1; z < 15; z++)
                {
                    ...
                    double moistureSignal = Math.Clamp(moisture / moistureClamp, 0.0, 1.0);
                    double moistureThreshold = 0.55 - stabilityWeight * 0.13;
                    ...
                    double roofStability = Math.Clamp(1.0 - roofThickness / 3.0, 0.0, 1.0);
                    double sealStrength = Math.Clamp((0.75 - roofThickness * 0.25) + moistureSignal * (0.35 + stabilityWeight * 0.15), 0.0, 1.0);
                    sealStrength = Math.Clamp(sealStrength * (0.85 + stabilityWeight * 0.2) * (0.9 + roofStability * 0.15), 0.0, 1.0);
        ...
        private HydrologyFieldCache GetHydrologyField(TerrainGenerationContext context, int[,] surfaceCache)
        {
            return context.GetOrAddMetadata(HydrologyFieldCacheKey, () =>
            {
                ...
                ApplyHydrologyFlowMemory(surfaceCache, hydrologyMask, flowAccumulation);
        ...
        private void ApplyHydrologyFlowMemory(int[,] surfaceCache, double[,] hydrologyMask, double[,] flowAccumulation)
        {
            if (_hydrologyFlowMemoryWeight <= 0.0 || hydrologyMask == null || flowAccumulation == null || surfaceCache == null)
            {
                return;
            }
            ...
            double weight = Math.Clamp(_hydrologyFlowMemoryWeight + _hydrologyFlowPersistence * 0.2, 0.0, 1.0);
            int blendRadius = Math.Max(1, Math.Min(2, _hydrologyEdgeBlendRadius));

            for (int x = 0; x < width; x++)
            {
                for (int z = 0; z < depth; z++)
                {
                    double hydrology = hydrologyCopy[x, z];
                    double normalizedFlow = Math.Clamp(flowCopy[x, z] / 6.0, 0.0, 1.5);
                    double hydrologySum = 0.0;
                    double flowSum = 0.0;
                    int samples = 0;
                    ...
                    for (int dx = -blendRadius; dx <= blendRadius; dx++)
                    {
                        for (int dz = -blendRadius; dz <= blendRadius; dz++)
                        {
                            ...
                            hydrologySum += hydrologyCopy[sampleX, sampleZ];
                            flowSum += Math.Clamp(flowCopy[sampleX, sampleZ] / 6.0, 0.0, 1.5);
                            samples++;
                        }
                    }
                    ...
                    double hydrologyAvg = hydrologySum / samples;
                    double flowAvg = flowSum / samples;
                    double slope = ComputeLocalRelief(surfaceCache, x, z, Math.Max(1, blendRadius + 1));
                    double basinWeight = Math.Clamp(1.0 - slope / 12.0, 0.0, 1.0);
                    double memory = Math.Clamp((hydrologyAvg * 0.65 + flowAvg * 0.35) * (0.85 + basinWeight * 0.35), 0.0, 1.5);
                    double blendedHydrology = Math.Clamp(hydrology * (1.0 - weight) + memory * weight, 0.0, 1.5);
                    hydrologyMask[x, z] = blendedHydrology;

                    double flowMemory = Math.Clamp((flowAvg * 0.7 + normalizedFlow * 0.3) * (0.9 + basinWeight * 0.25), 0.0, 1.5);
                    double baselineFlow = flowCopy[x, z];
                    double targetFlow = baselineFlow * (1.0 - weight * 0.25) + flowMemory * 6.0 * (weight * 0.25);
                    double flowClamp = Math.Max(baselineFlow * 1.25 + 0.5, baselineFlow + 2.0);
                    flowAccumulation[x, z] = Math.Clamp(targetFlow, 0.0, flowClamp);
                }
            }
        }
        ...
        private void SmoothRiverIntensity(double[,] riverIntensity, double[,] erosionRiskField, double[,] hydrologyMask, double[,] flowAccumulation)
        {
            ...
            int edgeRadius = Math.Max(1, _hydrologyEdgeBlendRadius);
            double edgeStabilityWeight = Math.Clamp(_hydrologyEdgeStabilityWeight, 0.0, 1.0);
            var scratch = new double[width, depth];
            ...
            for (int iteration = 0; iteration < iterations; iteration++)
            {
                for (int x = 0; x < width; x++)
                {
                    for (int z = 0; z < depth; z++)
                    {
                        ...
                        double seamHydro = SampleHydrologyAverage(hydrologyMask, x, z);
                        double centerHydrologyDelta = Math.Abs(hydrology - seamHydro);
                        int edgeDistance = Math.Min(Math.Min(x, z), Math.Min(width - 1 - x, depth - 1 - z));
                        double edgeRamp = edgeRadius > 0 ? Math.Clamp((edgeRadius - Math.Min(edgeDistance, edgeRadius)) / (double)edgeRadius, 0.0, 1.0) : 0.0;
                        double blend = Math.Clamp(
                            baseBlend
                            + hydrology * 0.2
                            + flow * 0.12
                            + maxAlignment * 0.2
                            + headwater * _riverHeadwaterStabilityWeight * 0.35
                            + centerHydrologyDelta * _hydrologyEdgeStabilityWeight * 0.08
                            + edgeRamp * edgeStabilityWeight * 0.12,
                            0.0,
                            0.95);
