    ...
    public class WorldManager
    {
        ...
        private HydrologyFieldCache GetHydrologyField(TerrainGenerationContext context, int[,] surfaceCache)
        {
            return context.GetOrAddMetadata(HydrologyFieldCacheKey, () =>
            {
                ...
                var hydrologyGradient = BuildHydrologyGradient(hydrologyMask, flowAccumulation, surfaceCache);
        ...
        private Vector2[,] BuildHydrologyGradient(double[,] hydrologyMask, double[,] flowAccumulation, int[,] surfaceCache)
        {
            ...
            Vector2 ClampMagnitude(Vector2 value)
            {
            ...
            for (int x = 0; x < width; x++)
            {
                ...
                for (int z = 0; z < depth; z++)
                {
                    ...
                    gradient[x, z] = ClampMagnitude(downhill);
                }
            }
            ...
            for (int x = 0; x < width; x++)
            {
                for (int z = 0; z < depth; z++)
                {
                    ...
                    float hydrologyBias = (float)Math.Clamp(0.35 + hydrology * 0.45 + flowMemory * 0.25 + slopeWeight * 0.15 + flowStrength * 0.15, 0.35, 0.98);
                    ...
                    smoothed[x, z] = ClampMagnitude(Vector2.Lerp(raw, averaged, blendScale));
                }
            }
        ...
        public void GenerateLakesInternal(TerrainGenerationContext context)
        {
            ...
            spawnWeight = Math.Clamp(spawnWeight + riparian * 0.25 + flow * 0.2 + hydrologyCoherence * 0.2 + gradientBias * 0.12 + lakeConfig.SpawnWeightBias, 0.0, 1.35);
            ...
            for (int dx = -radiusX - 3; dx <= radiusX + 3; dx++)
            {
                for (int dz = -radiusZ - 3; dz <= radiusZ + 3; dz++)
                {
                    ...
                    double edgeNoise = SimplexNoise.Generate(
                        ...
                        0.55,
                        91771);
                    double perturbation = edgeNoise * (0.08 + erosionRiskField[x, z] * 0.06);
