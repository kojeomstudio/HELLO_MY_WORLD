...
namespace GameServerApp.World
{
    public partial class WorldManager
    {
        ...
        public WorldManager(DatabaseHelper database, WorldSettings? worldSettings = null, WorldGenerationConfig? generationConfig = null, int worldId = 1, WorldSeedConfig? worldSeed = null)
        {
            ...
            // ?îÎìú ?úÎìú Ï¥àÍ∏∞?? ?úÍ≥µ???úÎìú ?êÎäî ?∞Ïù¥?∞Î≤†?¥Ïä§?êÏÑú Î°úÎìú, ?êÎäî ?àÎ°ú ?ùÏÑ±
            ...
            // ?úÎìúÎ•??¨Ïö©?òÏó¨ Random Ï¥àÍ∏∞??(Í≤∞Ï†ï???ùÏÑ±???ÑÌï®)
        ...
        /// <summary>
        /// Í∞úÏÑ†??3D ?ôÍµ¥ ?ùÏÑ± ?úÏä§??- ???êÏó∞?§ÎüΩÍ≥??§Ïñë???ôÍµ¥ Íµ¨Ï°∞
        /// </summary>
        public void GenerateCavesInternal(TerrainGenerationContext context)
        {
            ...
            // ?∏Ïù¥Ï¶?Í∏∞Î∞ò ?ôÍµ¥Ï∏?Ï∂îÍ? (?∞ÏÜç??Î≥¥Ïû•)
        ...
        {
            int wormCount = 1 + rand.Next(3); // 1~3Í∞úÏùò Î©îÏù∏ ??
            ...
            for (int w = 0; w < wormCount; w++)
            {
                ...
                double y = rand.Next(15, 55); // ??ÍπäÏ? ÏßÄ?òÎ???
                double z = rand.Next(16);
                int steps = 100 + rand.Next(80); // ??Í∏??ôÍµ¥
                double yaw = rand.NextDouble() * Math.PI * 2.0;
                double pitch = (rand.NextDouble() - 0.5) * 0.4;
                double baseRadius = 2.0 + rand.NextDouble() * 1.5; // Í∏∞Î≥∏ Î∞òÏ?Î¶?

                for (int s = 0; s < steps; s++)
                {
                    // ?ôÏ†Å?ºÎ°ú Î≥Ä?òÎäî Î∞òÏ?Î¶?(?ìÏñ¥ÏßÄÍ≥?Ï¢ÅÏïÑÏßÄ???®Í≥º)
                    ...
                    // Í∞Ä????Í≥µÍ∞Ñ(Î∞? ?ùÏÑ±
                    if (s > 20 && rand.NextDouble() < 0.05) // 5% ?ïÎ•†
                    {
                    ...
                    // ?¥Îèô
                    double speed = 0.8 + rand.NextDouble() * 0.4; // Í∞ÄÎ≥Ä ?çÎèÑ
        ...
        {
            int roomCount = rand.Next(2, 6); // 2~5Í∞úÏùò ?åÌòï Î∞?
        ...
        {
            if (rand.NextDouble() < 0.3) // 30% ?ïÎ•†Î°??òÏßÅÍ∞??ùÏÑ±
            {
        ...
        /// <summary>
        /// ?∏Ïù¥Ï¶?Í∏∞Î∞ò ?ôÍµ¥Ï∏?- ?∞ÏÜç???∏Ïù¥Ï¶??ÑÎìúÎ•??¨Ïö©?òÏó¨ Ï≤?Å¨ Í≤ΩÍ≥ÑÎ•??òÎäî ?ôÍµ¥???ïÏÑ±?úÎã§.
        /// Í∞úÏÑ†: Ïπ®Ïàò ?ôÍµ¥(Flooded Caves) Í∏∞Îä•??Ï∂îÍ??òÏó¨ ?πÏ†ï ?íÏù¥ ?¥Ìïò, Í∑∏Î¶¨Í≥??òÎ¨∏?ôÏ†Å ?îÏù∏???∞Îùº Î¨ºÎ°ú Ï±ÑÏõåÏß??ôÍµ¥???ùÏÑ±?©Îãà??
        /// </summary>
            ...
            Vector2[,] hydrologyGradientField)
        {
            ...
            for (int x = 0; x < 16; x++)
            {
                ...
                for (int z = 0; z < 16; z++)
                {
                    ...
                    // Í∞úÏÑ†: ?òÎìúÏΩîÎî©??Í∞??Ä??_caveSettings ?¨Ïö©
                    ...
                    // ?†Í∑ú: Ïπ®Ïàò ?ôÍµ¥???ÑÌïú ?∏Ïù¥Ï¶?Í∞?Í≥ÑÏÇ∞
                    ...
                    for (int y = 8; y < 120; y++)
                    {
                        ...
                        if (density < dynamicThreshold)
                        {
                            ...
                            // ?†Í∑ú: Ïπ®Ïàò ?ôÍµ¥ Î°úÏßÅ
                            // yÏ¢åÌëúÍ∞Ä ?¥ÏàòÎ©¥Î≥¥????≥†, Ïπ®Ïàò ?ôÍµ¥ ?∏Ïù¥Ï¶?Í∞íÏù¥ ?πÏ†ï ?ÑÍ≥ÑÍ∞íÏùÑ ?òÏùÑ Í≤ΩÏö∞ ?ôÍµ¥??Î¨ºÎ°ú Ï±ÑÏõÅ?àÎã§.
            ...
            for (int x = 0; x < 16; x++)
            {
                for (int z = 0; z < 16; z++)
                {
                    ...
                    while (y > 8)
                    {
                        ...
                        {
                            // ?¨Í∏∞?úÎäî Ïπ®Ïàò ?ôÍµ¥Í≥??¨Î¶¨, ?ïÏ? ?ÖÎç©?¥Îßå ?ùÏÑ±?òÎ?Î°?Í∏∞Ï°¥ Î°úÏßÅ ?†Ï?
        ...
        private void CarveRoom(ChunkData chunk, int centerX, int centerY, int centerZ, int size)
        {
            ...
            {
                for (int dy = -size/2; dy <= size/2; dy++) // Î∞©Ï? ?òÌèâ?ÅÏúºÎ°????ìÍ≤å
                {
                    for (int dz = -size; dz <= size; dz++)
                    {
                        ...
                        {
                            double dist = Math.Sqrt(dx*dx + dy*dy*1.5 + dz*dz); // ?òÏßÅ ?ïÏ∂ï
                            if (dist <= size)
                            {
        ...
        /// <summary>
        /// Í∞úÏÑ†???òÏ†Ñ ?ùÏÑ± ?úÏä§??- ??Î≥µÏû°?òÍ≥† ?§Ïñë??Íµ¨Ï°∞???òÏ†Ñ
        /// </summary>
        public void GenerateDungeonsInternal(TerrainGenerationContext context)
        {
            ...
            if (rand.NextDouble() > 0.15) return; // 15% ?ïÎ•†Î°?Ï¶ùÍ?
        ...
        {
            int roomCount = 2 + rand.Next(3); // 2~4Í∞?Î∞?
        ...
        private void GenerateMazeDungeon(ChunkData chunk, Random rand)
        {
            ...
            int mazeSize = 8; // 8x8 ÎØ∏Î°ú
            
            // Í∞ÑÎã®??ÎØ∏Î°ú ?ùÏÑ± (??Î≥µÏû°???åÍ≥†Î¶¨Ï¶ò?ºÎ°ú ?ïÏû• Í∞Ä??
        ...
        {
            // Í∞ÑÎã®??ÏßÅÏÑ† Î≥µÎèÑ (??Î≥µÏû°???∞Í≤∞ Î°úÏßÅ?ºÎ°ú ?ïÏû• Í∞Ä??
            int corridorLength = rand.Next(3, 8);
            int direction = rand.Next(4); // 0:Î∂? 1:?? 2:?? 3:??
        ...
        /// <summary>
        /// Í∞úÏÑ†??Í¥ëÎ¨º ?ùÏÑ± ?úÏä§??- ???ÑÏã§?ÅÏù¥Í≥?Í∑†Ìòï ?°Ìûå Î∂ÑÎ∞∞
        /// </summary>
        public void GenerateOresInternal(TerrainGenerationContext context)
        {
            ...
            // Í∞?Í¥ëÎ¨ºÎ≥ÑÎ°ú ?¨Ïã§?ÅÏù∏ ÍπäÏù¥?Ä ?¨Í????§Ï†ï
            GenerateOreType(chunk, rand, BlockType.CoalOre, 5, 50, 12, 6);      // ?ùÌÉÑ: ?∏Ï†ú?? ?¨Îü¨ Ï∏µÏóê??
            GenerateOreType(chunk, rand, BlockType.IronOre, 1, 40, 8, 4);       // Ï≤? Ï§ëÍ∞Ñ ÍπäÏù¥
            GenerateOreType(chunk, rand, BlockType.GoldOre, 1, 25, 4, 3);       // Í∏? ÍπäÏ? Í≥?
            GenerateOreType(chunk, rand, BlockType.DiamondOre, 1, 16, 2, 2);    // ?§Ïù¥?ÑÎ™¨?? Í∞Ä??ÍπäÏ? Í≥?
        }
        
        /// <summary>
        /// ?πÏ†ï Í¥ëÎ¨º Ï¢ÖÎ•òÎ•??ùÏÑ±
        /// </summary>
            ...
            int minY, int maxY, int maxVeins, int maxVeinSize)
        {
            ...
            for (int vein = 0; vein < veinCount; vein++)
            {
                ...
                // Í¥ëÎß• Î™®Ïñë ?ùÏÑ± (Íµ¨Ìòï???ÑÎãå Î∂àÍ∑úÏπôÌïú ?ïÌÉú)
                GenerateOreVein(chunk, rand, oreType, centerX, centerY, centerZ, veinSize);
            }
        }
        
        /// <summary>
        /// Í¥ëÎß•??Î∂àÍ∑úÏπôÌïú ?ïÌÉúÎ°??ùÏÑ±
        /// </summary>
            ...
            int centerX, int centerY, int centerZ, int size)
        {
            ...
            for (int i = 0; i < size - 1; i++)
            {
                ...
                // Í∏∞Ï°¥ Í¥ëÎ¨º Î∏îÎ°ù Ï§?Î¨¥Ïûë?ÑÎ°ú ?òÎÇò ?†ÌÉù
                var baseBlock = oreBlocks[rand.Next(oreBlocks.Count)];
                
                // 6Î∞©Ìñ• Ï§?Î¨¥Ïûë?ÑÎ°ú ?ïÏÇ∞
        ...
        // === ÎßàÏù∏?¨Îûò?ÑÌä∏ ?∏Îì§?¨Ïö© Ï∂îÍ? Î©îÏÑú?úÎì§ ===

        /// <summary>
        /// ?πÏ†ï Î∏îÎ°ù ?ïÎ≥¥ Í∞Ä?∏Ïò§Í∏?
        /// </summary>
        ...
        {
            // TODO: ?§Ï†ú Íµ¨ÌòÑ?êÏÑú???∞Ïù¥?∞Î≤†?¥Ïä§?êÏÑú ?îÌã∞??Ï°∞Ìöå
            // ?ÑÏû¨??Îπ?Î¶¨Ïä§??Î∞òÌôò
