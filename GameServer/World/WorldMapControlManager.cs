...
namespace GameServerApp.World
{
    ...
    public sealed class WorldMapControlManager
    {
        ...
        private readonly double configuredQueuePhreaticResonanceWeight;
        ...
        public WorldMapControlManager(WorldMapControlSettings settings, WorldGenerationConfig generationConfig, WorldSettings worldSettings)
        {
            ...
            configuredQueueStalePruneEmergencyMultiplier = Math.Clamp(
                ...
                1.0,
                3.0);
            configuredQueueAlluvialRelayWeight = Math.Clamp(
                this.settings.QueueAlluvialRelayWeight <= 0.0 ? 0.82 : this.settings.QueueAlluvialRelayWeight,
                0.0,
                1.5);
            ...
            configuredQueueHyporheicExchangeWeight = Math.Clamp(
                ...
                0.0,
                1.5);
            configuredQueuePhreaticResonanceWeight = Math.Clamp(
                this.settings.QueuePhreaticResonanceWeight <= 0.0 ? 0.98 : this.settings.QueuePhreaticResonanceWeight,
                0.0,
                1.6);
        ...
        private int GetAdaptiveQueueLimit()
        {
            ...
            double seamResilienceScale = WorldMapQueuePolicy.ComputeHydrologySeamResilienceScale(
                ...
                0.62,
                1.2);
            double alluvialWeight = configuredQueueAlluvialRelayWeight;
            ...
            double phreaticWeight = Math.Clamp(
                configuredQueuePhreaticResonanceWeight * (0.44 + configuredQueueHyporheicExchangeWeight * 0.32 + configuredQueueKarstSpillwayWeight * 0.24),
                0.0,
                1.6);
        ...
        private int ComputeNearChunkKeepCount(int fallbackBase, QueuePressureBand pressureBand, double queueLoadSnapshot)
        {
            ...
            int seamAdjusted = (int)Math.Round(
                nearKeep * Math.Clamp(seamResilienceScale * alluvialRelayScale * karstFloodplainRelayScale * spillwayQueueScale * aquiferConduitQueueScale * subterraneanRechargeQueueScale * hyporheicExchangeQueueScale * phreaticResonanceQueueScale, 0.76, 1.3));
