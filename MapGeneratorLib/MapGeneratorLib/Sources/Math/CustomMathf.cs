...
namespace MapGenLib
{
    public class CustomMathf
    {
        ...
        {
            if (CustomMathf.Abs(target - current) <= maxDelta)
                return target;
            return current + CustomMathf.Sign(target - current) * maxDelta;
        }
        ...
        {
            return Clamp(t - CustomMathf.Floor(t / length) * length, 0.0f, length);
        }
        ...
        public static float PingPong(float t, float length)
        {
            ...
            return length - CustomMathf.Abs(t - length);
        }
        ...
        {
            float delta = CustomMathf.Repeat((target - current), 360.0F);
            if (delta > 180.0F)
                delta -= 360.0F;
            return delta;
        }
        ...
        internal static bool LineIntersection(CustomVector2 p1, CustomVector2 p2, CustomVector2 p3, CustomVector2 p4, ref CustomVector2 result)
        {
            float bx = p2.x - p1.x;
            float by = p2.y - p1.y;
            float dx = p4.x - p3.x;
        ...
        internal static bool LineSegmentIntersection(CustomVector2 p1, CustomVector2 p2, CustomVector2 p3, CustomVector2 p4, ref CustomVector2 result)
        {
            float bx = p2.x - p1.x;
            float by = p2.y - p1.y;
            float dx = p4.x - p3.x;
