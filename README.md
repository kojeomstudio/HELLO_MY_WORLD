...
## Recent Updates
- 2026-02-13: Shared improved hydrology/flow masks between the server chunk pipeline and Unity previews (`ImprovedTerrainCoordinator`, `WorldManager.GenerateImproved*`, `EnhancedWorldMapController`), reloading map-control pipelines on hash drift in `WorldMapControlManager`, tightened proto descriptor checks in `ProtocolValidator`, and refreshed the core/content/util matrix (`config/minecraft_feature_core_content_util.json`, `docs/minecraft_feature_core_content_util_2026-02-13.md`).
