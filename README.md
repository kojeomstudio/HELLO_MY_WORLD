...
## Recent Updates
- 2026-02-22: Added hydrology/flow-aware river bank, wetland, and lake outflow shaping on server (`GameServer/World/Generation/EnhancedTerrainGenerationPipeline.cs`), Unity preview (`Assets/Scripts/Minecraft/World/EnhancedTerrainGenerator.cs`), and map previews (`MapGeneratorLib/.../WorldGenAlgorithms.cs`); tightened world-map control reloads on profile timestamp drift (`GameServer/World/WorldMapControlManager.cs`); enforced C# namespace parity for EnhancedMinecraft protobuf bindings (`SharedProtocol/EnhancedMinecraft/ProtocolValidator.cs`); and published the refreshed core/content/util rollout (`config/minecraft_feature_client_server_core_content_util_2026-02-22.json`).
