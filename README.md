...
## Recent Updates
- 2025-11-17: Completed the hydrology feedback loop by wiring `ApplyCaveHydrologyErosion`, `ApplyRiverHydrologyFeedback`, and `StabilizeLakeCatchments` into `WorldManager` and retuning MapGeneratorLib with matching clay/sand beds. Unity now executes `ProtoRuntime.EnsureInitialized()` via `EnhancedChunkPayloadBridge`, so protobuf mismatches are caught in both tooling and the dedicated server. See `docs/world-generation.md` and the new `docs/minecraft_feature_execution_2025-11-16.md` log for the rollout order.
