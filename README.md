...
## Recent Updates
- 2025-12-30: Headwater-aware river intensity smoothing (`RiverHeadwaterStabilityWeight`), lake inflow routing (`LakeInflowBlendWeight`), and moisture-retention-biased cave stability (`MoistureRetentionWeight`) are mirrored between `GameServer/World/WorldManager.cs` and `MapGeneratorLib/.../WorldGenAlgorithms.cs` with matching JSON knobs in `config/world.json` and Unity `WorldConfigData.json`. Chunk handlers now call `ProtoRuntime.EnsureInitialized()` + `ProtocolValidator.ValidateChunkContracts()` so EnhancedMinecraft payload parsing fails fast on stale DTOs. See `docs/minecraft_feature_core_content_util_2025-12-19.md` and `docs/minecraft_feature_core_content_util_latest.md`.
