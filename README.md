...
## Recent Updates
- 2025-12-30: Collapsed the duplicated configuration models into a single JSON-backed `DataDrivenConfigManager`, regenerated the world map control profile at server startup with map-control settings, tightened the protobuf gateway around `ProtocolRegistry`, and relaxed river/lake seams with riparian bank feathering in the enhanced terrain pipeline (plus simplified improved generators coordinated via `ImprovedTerrainCoordinator`). Latest core/content/util rollout lives in `docs/minecraft_feature_core_content_util_2025-12-30-plan.md`.
