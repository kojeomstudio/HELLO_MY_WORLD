...
- 2026-01-13: Hydrology gradient stabilization is now anisotropic (downhill/flow aligned) in both `GameServer/World/WorldManager.cs` and `MapGeneratorLib/.../WorldGenAlgorithms.cs`, reducing river/lake/cave seam drift across chunks and keeping Unity previews in sync. Server startup now calls `ProtocolValidator.ValidateEnhancedContracts()` to fail fast on stale protobuf assets, and the core/content/utility backlog was refreshed in `minecraft_features_implementation_plan.md`.
