...
**Hydrology Signature:** `2026-02-03-hydrology-riverlake-v12`  
**Profile Version:** 14 (`config/world.json`, `config/world_map_control_profile.json`, `Assets/StreamingAssets/world-config.json`)  
**Feature Manifest:** `config/minecraft_feature_core_content_util_2026-02-03-session-40.json`

## Worldgen Changes (Server + MapGeneratorLib)
- Hydrology stability: added in-place stabilization + gradient coupling for hydrology/flow masks before carving (`EnhancedTerrainGenerationPipeline.StabilizeHydrologyFields`), using variance clamps, flow memory, water-table bias, and new continuity smoothing.
...
- MapGeneratorLib alignment: constants raised to hydrology v12 values and `EnhanceHydrologyContinuity` keeps client hydrology/flow masks stable across chunk seams.
...
- Dummy client logs hydrology signature + registered packet count alongside registry/fingerprint assertions; writes probe + reference reports (`reports/proto_probe_report.json`, `config/proto_reference_report.json`).
