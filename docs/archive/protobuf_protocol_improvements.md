...
- `ProtocolValidator.ValidateHandlerBindings()` cross-checks registered EnhancedMinecraft handlers against the generated contract types and warns when non-optional packets lack handlers, catching stale `using` directives or mismatched contracts early in startup.
...
- `ProtoDiagnostics.LogHandlerCoverage()` now reports EnhancedMinecraft handler gaps at startup (hooked in `GameServer.RegisterMinecraftHandlers`), ensuring generated DTOs registered in `ProtocolRegistry` are actually bound to runtime handlers.
