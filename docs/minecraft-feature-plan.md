...
| F-11 | Remote Player Entity Sync & Interpolation | Broadcast player spawn/update/despawn, smooth avatars, cull off-screen actors | In progress (velocity sanitisation + spawn/update loop) | In progress (RemoteEntityManager predictive smoothing, culling pending) | Task-11C now focuses on distance culling and pooling after velocity damping. |
