# Minecraft Client/Server Feature Sequence (2025-11-15)
...
| F-07 | Networking/Protocol Validation | `SharedProtocol/EnhancedMinecraft/*` (new registry metadata, diagnostics, fingerprint guard). | `Assets/Generated/Protobuf`, `Assets/Scripts/Minecraft/Core/EnhancedChunkPayloadBridge.cs`. | ✅ Enhanced | Registry metadata now drives ProtoDiagnostics; sequential follow-up is wiring automated regeneration checks into CI. |
...
3. **Terrain alignment (this change)** – F-06 delivers identical cave/rive/lake geometry across server and MapGeneratorLib while F-07 ensures protobuf contracts stay valid. These unblock biome/tooling work.
4. **Hydrology polish (next)** – F-08 expands wetlands/vegetation once the new hydrology masks settle.
5. **Observability (near future)** – F-09 adds residency metrics using the enhanced protocol payloads.
6. **Visual fidelity (future)** – F-10 covers lighting and biome-specific tweaks once the above are stable.
