...
| Feature | Server responsibilities | Client responsibilities | Data / Protocol / Config |
| --- | --- | --- | --- |
| Terrain & hydrology (caves/rivers/lakes) | Heightmap + hydrology generation, caves, dungeons, river/lake carving with JSON thresholds | Render streamed chunks; MapGeneratorLib preview uses same knobs | World JSON (`Water.*` incl. `HydrologySmooth*`, `HydrologyEdgeBlendRadius`, `HydrologyFlowPersistence`, `HydrologySeamRelax*`, `RiverNoiseScale`, `RiverDepth`; `Caves.*`), Unity `WorldConfigData.json`; chunks carry block payloads |
...
| Feature | Server responsibilities | Client responsibilities | Data / Protocol / Config |
| --- | --- | --- | --- |
| Config/tuning alignment | Load `server-config.json`, `config/world.json`; expose to worldgen and networking | Mirror configs in `Resources/TextAsset/GameWorld/WorldConfigData.json` for previews/UI | Keep seeds, hydrology/cave toggles, seam relax/smoothing knobs, erosion weights, and room/time limits in JSON |
...
Hydrology gradient/seam tuning (`HydrologyShorePush`, `HydrologySlopePenalty`, `HydrologyFlowGain`, `HydrologyContinuityWeight` plus the smooth iterations/blend and erosion weights) now lives alongside the water knobs in both `config/world.json` and `Resources/TextAsset/GameWorld/WorldConfigData.json`, keeping seam smoothing and shoreline stabilization identical between WorldManager and MapGeneratorLib previews. Edge-aware seam blending and flow persistence use the new `HydrologyEdgeBlendRadius` and `HydrologyFlowPersistence` settings on both sides, and the new seam relaxation knobs (`HydrologySeamRelaxIterations`, `HydrologySeamRelaxBlend`) pull edge rings toward interior hydrology/flow so rivers, lakes, and caves stay stable at chunk borders. MapGeneratorLib now mirrors the server’s hydrology smoothing and flow-persistence weighting before normalization, reducing preview/server drift. Erosion-risk smoothing reuses those hydrology smooth settings, and river frequency/base depth remain data-driven (`RiverNoiseScale`, `RiverDepth`) so server streams and Unity previews pick up the same meander spacing and channel depth without code changes. Lake spawn pressure (`LakeSpawnWeightBias`), shoreline blending (`LakeShorelineBlend`), and river/lake bank carving weights (`RiverBankErosionWeight`, `LakeRimErosionWeight`) now drive both the server and Unity previews so terrain silhouettes match before/after streaming.
...
- [x] Hydrology seam relaxation (`HydrologySeamRelaxIterations/Blend`) runs after normalization on both server and Unity tooling to keep river/lake/cave silhouettes stable along chunk borders.
...
- [x] Edge-aware hydrology seam blending (`HydrologyEdgeBlendRadius`) and flow persistence (`HydrologyFlowPersistence`) align river/lake/cave masks across chunk borders on server and client previews.
...
- [x] Chunk unload descriptors (`ChunkUnloadNotification`/`ChunkUnloadAck`) validated to catch stale EnhancedMinecraft protobuf references before chunk streaming.
