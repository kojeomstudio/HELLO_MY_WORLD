...
Current feature split and rollout order across server and Unity client. Updated 2025-12-16; adds configurable river intensity smoothing (`RiverIntensitySmoothIterations`/`RiverIntensitySmoothBlend`) and moisture-aware cave support density (`CaveSupportDensity`) shared by server and MapGeneratorLib to keep wet caverns stable and rivers less jagged between chunks.

## Core (authority, world map control, protocol)
| Feature | Server (files) | Client (files) | Data / Protocol / Notes |
| --- | --- | --- | --- |
| World map control & chunk streaming | `GameServer/World/WorldManager.cs`, `Generation/*`, `ChunkPayloadBuilder` streaming + persistence; room-aware routing in `SessionManager.cs`; flow-aligned hydrology seam bias for chunk edges plus river intensity smoothing before shipment | `Assets/MyAssets/Scripts/GameWorld/WorldAreaManager.cs`, `SubWorld.cs` request/unload + apply payloads; MapGeneratorLib mirrors hydrology seam blending/bias and river smoothing; sky/time/weather updates | Config: `config/world.json`, `server-config.json`; Unity mirror `Assets/MyAssets/Resources/TextAsset/GameWorld/WorldConfigData.json` (includes `HydrologyEdgeFlowBias`, `RiverIntensitySmoothIterations`, `RiverIntensitySmoothBlend`). Proto: `ChunkDataRequest/Response`, `ChunkUnloadNotification/Ack`, `WorldInfo`, `TimeUpdate`, `WeatherChange`. |
...
| Feature | Server (files) | Client (files) | Data / Protocol / Notes |
| --- | --- | --- | --- |
| Terrain & hydrology (caves/rivers/lakes) | `WorldManager` hydrology mask + flow + erosion with flow-aligned multi-ring seam smoothing/relaxation; configurable river intensity smoothing and moisture-aware cave support density for wet caverns; generation stages in `Generation/Stages/*` | MapGeneratorLib mirrors hydrology/cave/river/lake masks for Unity preview with the same flow-aware seam smoothing, river smoothing, and cave support density to keep previews identical to streamed chunks | Config knobs: `Water.*` (`HydrologySmooth*`, `HydrologyEdgeBlendRadius`, `HydrologyEdgeFlowBias`, `HydrologyFlowPersistence`, `HydrologySeamRelax*`, `RiverNoiseScale`, `RiverDepth`, `RiverIntensitySmoothIterations`, `RiverIntensitySmoothBlend`, erosion weights) + `Caves.*` stability/noise settings including `CaveSupportDensity`. |
...
| Feature | Server (files) | Client (files) | Data / Protocol / Notes |
| --- | --- | --- | --- |
| Config/tuning alignment | Load `config/world.json` + `server-config.json`; expose to worldgen/networking | Mirror values in `WorldConfigData.json` for previews/UI | Keep seeds/hydrology/cave toggles, seam smoothing, `HydrologyEdgeFlowBias`, river intensity smoothing, cave support density, erosion weights in JSON; document new keys. |
...
1) Core/authority: chunk routing + flow-aligned seam blending, river intensity smoothing, and proto validation (registry + parser + assembly) before accepting traffic (Chunk/World/Time/Weather/Unload messages).  
2) Content/worldgen: hydrology-driven caves/rivers/lakes with flow-biased seam smoothing, configurable river smoothing, and moisture-aware cave supports using JSON knobs shared with Unity previews.  
3) Utility/tooling: keep configs, generated protobufs, and data tables in lockstep; add metrics/recordings around world map control, hydrology quality, and seam stability.  
