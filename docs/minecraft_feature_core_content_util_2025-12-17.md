Current feature split and rollout order across server and Unity client. Updated 2025-12-17; adds hydrology edge variance clamping shared by WorldManager and MapGeneratorLib to keep rivers/lakes/caves continuous at chunk seams, and boots EnhancedMinecraft protobuf validation via Program startup before any network traffic.

## Core (authority, world map control, protocol)
| Feature | Server (files) | Client (files) | Data / Protocol / Notes |
| --- | --- | --- | --- |
| World map control & chunk streaming | `GameServer/World/WorldManager.cs`, `Generation/*`, `ChunkPayloadBuilder` streaming + persistence; edge-variance clamp on hydrology seams plus river intensity smoothing before shipment | `Assets/MyAssets/Scripts/GameWorld/WorldAreaManager.cs`, `SubWorld.cs` request/unload + apply payloads; MapGeneratorLib mirrors hydrology seam blending/variance clamp and river smoothing; sky/time/weather updates | Config: `config/world.json`, `server-config.json`; Unity mirror `Assets/MyAssets/Resources/TextAsset/GameWorld/WorldConfigData.json` (includes `HydrologyEdgeVarianceClamp`, `RiverIntensitySmoothIterations`, `RiverIntensitySmoothBlend`). Proto: `ChunkDataRequest/Response`, `ChunkUnloadNotification/Ack`, `WorldInfo`, `TimeUpdate`, `WeatherChange`. |
...
| Feature | Server (files) | Client (files) | Data / Protocol / Notes |
| --- | --- | --- | --- |
| Terrain & hydrology (caves/rivers/lakes) | `WorldManager` hydrology mask + flow + erosion with flow-aligned seam smoothing/relaxation; edge-variance clamp stabilizes river/lake/cave seams and preserves catchments; generation stages in `Generation/Stages/*` | MapGeneratorLib mirrors hydrology/cave/river/lake masks with the same seam clamp so Unity previews match streamed chunks | Config knobs: `Water.*` (`HydrologySmooth*`, `HydrologyEdgeBlendRadius`, `HydrologyEdgeFlowBias`, `HydrologyEdgeVarianceClamp`, `HydrologyFlowPersistence`, `HydrologySeamRelax*`, `RiverNoiseScale`, `RiverDepth`, `RiverIntensitySmoothIterations`, `RiverIntensitySmoothBlend`, erosion weights) + `Caves.*` stability/noise settings including `CaveSupportDensity`. |
...
- ✅ Added hydrology edge-variance clamp shared by server + MapGeneratorLib and wired through JSON configs.
- ✅ Boot-time protobuf validation now runs in `Program.Main` via `ProtoRuntime.EnsureInitialized()` to fail fast on stale generated DTOs.
- ⏩ Next: expose a small hydrology debug overlay in Unity to visualize seam clamps and catchments while tuning `HydrologyEdgeVarianceClamp` per biome.
