Current feature split and rollout order across server and Unity client. Updated 2025-12-20; adds flow-aligned/gradient-aware river smoothing, moisture/flow-biased cave supports, and hydrology warp sampling so Unity previews match server chunk streams with the same JSON knobs.

## Core (authority, world map control, protocol)
| Feature | Server (files) | Client (files) | Data / Protocol / Notes |
| --- | --- | --- | --- |
| World map control & chunk streaming | `GameServer/World/WorldManager.cs`, `Generation/*`, `ChunkPayloadBuilder` streaming/persistence; river intensity smoothing now uses flow-aware/gradient-penalized passes (`_riverFlowAlignmentWeight`, `_riverGradientPenalty`) before shipment | `Assets/MyAssets/Scripts/GameWorld/WorldAreaManager.cs`, `SubWorld.cs`; MapGeneratorLib mirrors river smoothing knobs + hydrology warp sampling (`HydrologyWarp*`) so Unity previews match server chunks; sky/time/weather updates | Config: `config/world.json`, `Assets/.../WorldConfigData.json` mirror `RiverFlowAlignmentWeight`, `RiverGradientPenalty`, `HydrologyWarpFrequency`, `HydrologyWarpAmplitude`, `CaveSupport*`. Proto: `ChunkDataRequest/Response`, `ChunkUnloadNotification/Ack`, `WorldInfo`, `TimeUpdate`, `WeatherChange`. |
...
| Feature | Server (files) | Client (files) | Data / Protocol / Notes |
| --- | --- | --- | --- |
| Terrain & hydrology (caves/rivers/lakes) | `WorldManager` hydrology mask + flow + erosion with flow-aligned seam smoothing/relaxation; river smoothing now anisotropic (`RiverFlowAlignmentWeight`/`RiverGradientPenalty`), cave supports are moisture/flow-biased (`SupportHydrationBias`/`SupportFlowBias`), hydrology warp keeps humidity sampling coherent across seams | MapGeneratorLib mirrors hydrology/cave/river/lake masks with the same anisotropic river smoothing, hydrology warp sampling, and moisture-biased support columns so Unity previews match streamed chunks | Config knobs: `Water.*` (hydrology seam/warp, flow persistence, river noise/depth/smoothing, anisotropy weights) + `Caves.*` stability/support knobs (`SupportDensity`, `SupportHydrationBias`, `SupportFlowBias`, stability weights) shared between server and Unity JSON. |
