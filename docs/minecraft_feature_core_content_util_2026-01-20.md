...
| Hydrology-guided caves/rivers/lakes | Shared hydrology/flow/gradient fields power cave stability, river smoothing, lake spawn masks | MapGeneratorLib consumes the same fields for offline previews | JSON: `Hydrology*`, `River*`, `Lake*`, `Caves.*` in `config/world.json` and Unity `WorldConfigData.json`. |
