...
Hydrology gradient/seam tuning (`HydrologyShorePush`, `HydrologySlopePenalty`, `HydrologyFlowGain`, `HydrologyContinuityWeight` plus the smooth iterations/blend and erosion weights) now lives alongside the water knobs in both `config/world.json` and `Resources/TextAsset/GameWorld/WorldConfigData.json`, keeping seam smoothing and shoreline stabilization identical between WorldManager and MapGeneratorLib previews. Erosion-risk smoothing now reuses those hydrology smooth settings, and river frequency/base depth remain data-driven (`RiverNoiseScale`, `RiverDepth`) so server streams and Unity previews pick up the same meander spacing and channel depth without code changes. Lake spawn pressure (`LakeSpawnWeightBias`), shoreline blending (`LakeShorelineBlend`), and river/lake bank carving weights (`RiverBankErosionWeight`, `LakeRimErosionWeight`) now drive both the server and Unity previews so terrain silhouettes match before/after streaming.
