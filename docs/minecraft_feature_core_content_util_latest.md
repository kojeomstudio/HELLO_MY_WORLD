Current feature split and rollout order across server and Unity client. Updated 2025-12-31; see `docs/minecraft_feature_core_content_util_2025-12-31.md` for the snapshot. This iteration adds flow-aligned anisotropic river smoothing (`RiverAnisotropyWeight`) mirrored between `GameServer/World/WorldManager.cs` and `MapGeneratorLib`, keeping river banks coherent across chunk seams while proto guards remain in place.
...
| --- | --- | --- | --- |
| World map control & chunk streaming | `GameServer/World/WorldManager.cs`, `Generation/*`, `ChunkPayloadBuilder` streaming/persistence; hydrology seam blend + edge stability + water-table-aware flow accumulation + hydrology gradient cache + flow-aligned anisotropic river smoothing + clamp run before river/lake carving to keep seams continuous and aligned to configured sea level | `Assets/MyAssets/Scripts/GameWorld/WorldAreaManager.cs`, `SubWorld.cs`; MapGeneratorLib mirrors seam blend + stability + water-table-aware flow accumulation/clamp and anisotropic river smoothing so Unity previews match server chunks; sky/time/weather updates | Config: `config/world.json`, `Assets/.../WorldConfigData.json` mirror `GlobalWaterLevel`, `HydrologyWaterTableClampWeight/Range/SlopeWeight`, `HydrologyEdge*`, `HydrologyFlowPersistence`, `RiverFlowAlignmentWeight`, `RiverGradientPenalty`, `RiverAnisotropyWeight`, `HydrologyWarpFrequency`, `HydrologyWarpAmplitude`, `CaveSupport*`. Proto: `ChunkDataRequest/Response`, `ChunkUnloadNotification/Ack`, `WorldInfo`, `TimeUpdate`, `WeatherChange`. |
...
| Feature | Server (files) | Client (files) | Data / Protocol / Notes |
| --- | --- | --- | --- |
| Config/tuning alignment | Load `config/world.json` + `server-config.json`; expose to worldgen/networking | Mirror values in `WorldConfigData.json` for previews/UI | New JSON knobs: `HydrologyWaterTableClampWeight/Range/SlopeWeight`, `HydrologyGradientWeight`, `RiverAnisotropyWeight`, `GlobalWaterLevel` mirrored into MapGeneratorLib, plus existing hydrology/cave/river tuning (`HydrologyEdge*`, `HydrologyWarp*`, `RiverFlowAlignmentWeight`, `RiverGradientPenalty`, `SupportHydrationBias`, `SupportFlowBias`). |
...
- [x] Flow-aligned anisotropic river smoothing controlled by `RiverAnisotropyWeight` (server + MapGeneratorLib) keeps banks coherent while preserving downstream momentum.
- [x] Hydrology gradient cache (WorldManager + MapGeneratorLib) steers caves/rivers/lakes via `HydrologyGradientWeight`; overflow routing now follows downhill hydrology seams.
...
- Next: surface a hydrology/gradient debug overlay in Unity and tune `HydrologyGradientWeight` against `RiverFlowAlignmentWeight` using seam QA captures.
