Current feature split and rollout order across server and Unity client. Updated 2025-12-28; see `docs/minecraft_feature_core_content_util_2025-12-28.md` for the snapshot. This iteration adds a slope-aware water-table clamp (`HydrologyWaterTableSlopeWeight`) shared between WorldManager and MapGeneratorLib so rivers/lakes/caves bias toward sea level without flooding cliffs, and extends ProtocolValidator coverage to `PlayerActionRequest/Response/ActionResult` alongside the existing world/time/weather/status checks.
...
| Protocol registry & guards | `SharedProtocol/EnhancedMinecraft/ProtocolValidator.cs`, `ProtoRuntime.EnsureInitialized()` invoked in `Program.cs`; registry/descriptor/parser validation + assembly consistency guard now also checks `PlayerActionRequest/Response/ActionResult` plus `WeatherUpdateBroadcast.change_timestamp` alongside status/world/time/transfer descriptors | Unity tooling logs proto drift; regenerate DTOs when proto changes | Run `protoc -I proto --csharp_out=Assets/Generated/Protobuf proto/*.proto` then `dotnet build SharedProtocol/SharedProtocol.csproj`. |
...
- [x] Slope-aware water-table clamp (`HydrologyWaterTableSlopeWeight`) shared between `WorldManager` and MapGeneratorLib biases hydrology/flow toward `GlobalWaterLevel` while avoiding cliff flooding, keeping rivers/lakes/caves stable at chunk seams.
- [x] MapGeneratorLib uses the Unity world config water level (no fixed constant) and mirrors slope-aware clamping so previews stay aligned to server world map control.
- [x] ProtocolValidator asserts `PlayerActionRequest/Response/ActionResult` plus `WeatherUpdateBroadcast.change_timestamp` alongside registry/descriptor/parser/assembly drift checks so stale protobuf bindings fail fast.
- [x] Configs kept in sync: `config/world.json`, `Assets/MyAssets/Resources/TextAsset/GameWorld/WorldConfigData.json`, `WorldConfigFile`, and `WorldGenerationConfig` cover hydrology/river/cave knobs including the new slope weight.
