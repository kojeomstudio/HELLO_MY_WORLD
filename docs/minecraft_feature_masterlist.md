...
| F-33 | Cave Vent Shafts & Moisture Loops | Add hydrology-weighted ventilation shafts, basalt rims, and drip pooling inside `WorldManager.GenerateCavesInternal` so tall caverns stay ventilated | MapGeneratorLib reproduces the same shaft logic so Unity previews, lighting, and FX planning see identical geometry | Done | Surface shaft density/depth tunables in config + MapGenerator UI. |
| F-34 | River Confluence Delta Fans | Use river intensity + flow accumulation to widen channels and deposit sandbars downstream of braided confluences (`AddRiverDeltaFans`) | Unity tooling mirrors the fan placement so art captures show the widened braids and sandbars before export | Done | Attach biome-aware foliage & telemetry counters to new sandbars (Task-34B). |
| F-35 | Lake Overflow Channels & Spill Regulators | Evaluate nearby relief/hydrology to carve overflow trenches and reconnect basins to the river network | MapGeneratorLib mirrors overflow channels so tooling screenshots match the dedicated server terrain | Done | Expose overflow debug overlays and channel-length tunables. |
