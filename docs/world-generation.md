...
- **2025-12-12 edge seam smoothing**: `BlendHydrologySeams` now samples a multi-neighbor ring (instead of a single opposing cell) with continuity weighting on both `WorldManager` and `MapGeneratorLib`, reducing river/lake/cave edge stair-steps at chunk borders and keeping hydrology masks stable for seam relaxation.
