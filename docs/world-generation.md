...
- Hydrology mask construction now factors edge distance, shoreline push, and flow persistence directly: valley weighting uses `HydrologyShorePush`, humidity combines dual noise octaves scaled by `RiverNoiseScale`, and edge falloff obeys `HydrologyEdgeBlendRadius`. Default knobs were bumped (`HydrologySmoothIterations` 2 → blend 0.6, edge radius 3, flow persistence 0.62, seam relax blend 0.5) in both `config/world.json` and `Assets/MyAssets/Resources/TextAsset/GameWorld/WorldConfigData.json`.
