...
- Edge-normalization + flow-memory passes now run on hydrology/flow/river/lake masks (`GameServer/World/Generation/EnhancedTerrainGenerationPipeline.cs`, Unity preview generator in `Assets/MyAssets/Scripts/GameWorld/WorldMapController.cs`) using `HydrologyEdgeNormalization*` and `HydrologyFlowMemoryWeight` to keep caves/rivers/lakes stitched at chunk seams.
- River and lake masks pick up the same edge-normalization before seam relax so wetlands/outflows stay continuous across map-control previews and server chunks.
- Enhanced protocol handler now asserts the generated EnhancedMinecraft registry is clean (`GameServer/Network/EnhancedProtocolHandler.cs`) before parsing/serializing packets.
...
## Hydrology Seam Normalization (2026-01-10)
- Added co-normalization for hydrology/flow at chunk edges using `HydrologyEdgeNormalizationBlend`, `HydrologyEdgeNormalizationIterations`, and `HydrologyFlowMemoryWeight` in both server (`EnhancedTerrainGenerationPipeline`, `ImprovedTerrainCoordinator`) and Unity (`EnhancedTerrainGenerator`) paths.
- Rivers/lakes/caves bias thresholds with seam memory (hydrology + flow) and edge normalization to reduce riparian seams; see `ImprovedRiverGenerator`, `ImprovedLakeGenerator`, and `ImprovedCaveGenerator` updates.
- World-map control previews consume the same normalized masks; profile hashes remain in `config/world_map_control_profile.json` and StreamingAssets `world-config.json`.
- Protobuf validation now covers chunk unload descriptors via `ProtocolValidator.ValidateEnhancedContracts()` to catch stale generated DTOs earlier.
- Feature roster stored at `config/minecraft_feature_client_server_core_content_util_2026-01-10-worldgen-edge.json` (doc: `docs/minecraft_feature_client_server_core_content_util_2026-01-10-worldgen-edge.md`) tracks the above changes.
